...
def loadFolder(folderPath):
    train_test_dataset = ImageFolder(folderPath, transform = ToTensor())
    images = train_test_dataset.imgs
    labels = train_test_dataset.targets
    transform = transforms.Compose([transforms.ToTensor()])
    images_tensor = torch.stack([transform(Image.open(image_path)) for image_path, _ in images])
    labels_tensor = torch.tensor(labels)
    train_test_dataset = TensorDataset(images_tensor, labels_tensor)
    return train_test_dataset

def main(dataset: str, train_path: str, test_path: str, arch_id: str, loss: str, opt: str, lr: float, max_steps: int, neigs: int = 0,
    ...
    train_dataset, test_dataset = loadFolder(train_path), loadFolder(test_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("train", type=str, help="data folder to train")
    parser.add_argument("test", type=str, help="data folder to test")
    ...
    main(dataset=args.dataset, train_path=args.train, test_path=args.test, arch_id=args.arch_id, loss=args.loss, opt=args.opt, lr=args.lr, max_steps=args.max_steps,
